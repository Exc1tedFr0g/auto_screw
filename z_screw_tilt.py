# Mechanical bed tilt calibration with single Z stepper and multiple bed screw steppers
#
# Based on original z_tilt by Kevin O'Connor <kevin@koconnor.net>
#
# Copyright (C) 2024-2025  Austin <Exc1tedFr0g@bilibili.com>
#
...
class ZAdjustHelper:
    def __init__(self, config, z_count, screw_steppers):
        self.printer = config.get_printer()
        self.name = config.get_name()
        self.z_count = z_count
        self.screw_steppers_id = screw_steppers
        self.screw_steppers = [self.printer.lookup_object("manual_stepper "+s) for s in screw_steppers]
        self.printer.register_event_handler("klippy:connect",
                                            self.handle_connect)
    def handle_connect(self):
        if len(self.screw_steppers) != self.z_count - 1:
            raise self.printer.config_error(
                "%s z_positions needs exactly %d items" % (
                    self.name, len(self.screw_steppers) + 1))
        if len(self.screw_steppers) < 1:
            raise self.printer.config_error(
                "%s requires multiple z steppers" % (self.name,))
    def adjust_steppers(self, adjustments):
        toolhead = self.printer.lookup_object('toolhead')
        gcode = self.printer.lookup_object('gcode')
        curpos = toolhead.get_position()
        # Report on movements
        stepstrs = ["%s = %.6f" % (s, a)
                    for s, a in zip(self.screw_steppers_id, adjustments[1:])]
        msg = "Making the following Z adjustments:\n%s" % ("\n".join(stepstrs),)
        gcode.respond_info(msg)
        # Setting screw steppers to pos 0
        for s in self.screw_steppers:
            s.do_set_position(0)
        # Move each screw stepper
        positions = [(a, s) for a, s in zip(adjustments[1:], self.screw_steppers)]
        for i in range(len(positions)):
            stepper_offset, stepper = positions[i]
            adjust = stepper_offset - adjustments[0]
            try:
                stepper.do_move(-adjust, 0.5, 0., 1)
            except:
                logging.exception("ZAdjustHelper adjust_steppers")
                raise
        curpos[2] = curpos[2] - adjustments[0]
...
class ZTilt:
    def __init__(self, config):
        ...
        self.screw_steppers = config.getlist('screw_steppers', sep=',')
        self.retry_helper = RetryHelper(config)
        self.probe_helper = probe.ProbePointsHelper(config, self.probe_finalize)
        self.probe_helper.minimum_points(2)
        self.z_status = ZAdjustStatus(self.printer)
        self.z_helper = ZAdjustHelper(config, len(self.z_positions), self.screw_steppers)
        # Register Z_SCREW_TILT_ADJUST command
        gcode = self.printer.lookup_object('gcode')
        gcode.register_command('Z_SCREW_TILT_ADJUST', self.cmd_Z_SCREW_TILT_ADJUST,
                               desc=self.cmd_Z_SCREW_TILT_ADJUST_help)
    cmd_Z_SCREW_TILT_ADJUST_help = "Adjust the Z tilt with screws"
    def cmd_Z_SCREW_TILT_ADJUST(self, gcmd):
        self.z_status.reset()
    ...
    def probe_finalize(self, offsets, positions):
        ...
        self.z_helper.adjust_steppers(adjustments)
